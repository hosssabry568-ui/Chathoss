...
let partnerOnlineUnsub = null;
let selectedGroupMembers = new Set(); // أعضاء المجموعة المختارون
let contextMenuMsgId  = null;   // الرسالة المحددة في قائمة السياق
let contextMenuIsMe   = false;  // هل الرسالة للمستخدم الحالي
let ctxFullMsgData    = null;   // بيانات الرسالة الكاملة (للرد والتوجيه)

// ── الرد (Reply) ──
let currentReply = null; // { id, text, senderName }
...
function initSplash() {
  ...
  if (!splash) return;
  // انتظر 2.8 ثانية ثم اخفِ الانترو
  setTimeout(() => {
...
function renderChatsList(docs) {
  ...
  docs.forEach(d => {
    ...
    if (isGroup) {
      partnerName   = data.groupName || 'مجموعة';
      partnerAvatar = data.groupAvatar || generateAvatar(partnerName, '128C7E');
      partnerId     = null;
    } else {
      partnerId     = data.members.find(m => m !== currentUser.uid);
      const names   = data.memberNames   || {};
      const avatars = data.memberAvatars || {};
      partnerName   = names[partnerId]   || 'مستخدم';
      partnerAvatar = avatars[partnerId] || generateAvatar(partnerName);
    }
    ...
    const isOnline = !isGroup && data.memberOnline ? data.memberOnline[partnerId] : false;
    ...
    item.innerHTML = `
      <div class="chat-item-avatar">
        ...
        ${isGroup  ? '<div class="group-item-badge">👥</div>' : ''}
      </div>
    ...
    `;
    item.addEventListener('click', () => openChat(d.id, isGroup
      ? { uid: d.id, name: partnerName, avatar: partnerAvatar, isGroup: true, groupName: partnerName }
...
async function openChat(chatId, partner) {
  ...
  const displayName = partner.isGroup ? (partner.groupName || partner.name) : partner.name;
  setText('chat-partner-name', displayName);
  setAttr('chat-partner-avatar', 'src', partner.avatar || generateAvatar(displayName));

  // حالة المجموعة
  if (partner.isGroup) {
    const membersSnap = await FB.getDoc(FB.doc(FB.db, 'chats', chatId));
    if (membersSnap.exists()) {
      const d = membersSnap.data();
      const count = (d.members || []).length;
      const statusEl = document.getElementById('partner-status-text');
      if (statusEl) {
        statusEl.textContent = `${count} أعضاء`;
        statusEl.classList.add('group-status');
      }
      document.getElementById('partner-online-dot').style.display = 'none';
    }
  }
  ...
  setTimeout(() => markMessagesAsRead(chatId), 1000);
...
function loadMessages(chatId) {
  ...
  // جلب clearedAt أولاً ثم الاستماع للرسائل
  FB.getDoc(FB.doc(FB.db, 'chats', chatId)).then(chatSnap => {
    const chatData  = chatSnap.exists() ? chatSnap.data() : {};
    const clearedTs = chatData.clearedAt?.[currentUser?.uid] || null;
    ...
    messagesUnsub = FB.onSnapshot(q, (snap) => {
      ...
      snap.docs.forEach(d => {
        ...
        // تصفية الرسائل الأقدم من clearedAt (مسح الشات لي فقط)
        if (clearedTs) {
          const clearedDate = clearedTs.toDate?.() || new Date(clearedTs);
          if (msgDate <= clearedDate) return;
        }
...
function buildMessageElement(id, data, isMe, date) {
  ...
  const isGroup = currentPartner && currentPartner.isGroup;

  // اسم المرسل في المجموعات
  let senderHtml = '';
  if (isGroup && !isMe && data.senderName) {
    senderHtml = `<div class="message-sender-name">${escapeHtml(data.senderName)}</div>`;
  }

  // شارة "محوّل"
  const forwardedBadge = data.forwarded
    ? `<div class="forwarded-badge">
        <svg width="11" height="11" viewBox="0 0 24 24" fill="none"><path d="M15 17l5-5-5-5" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M20 12H9a5 5 0 0 0-5 5v1" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
        محوّل
       </div>` : '';
  ...
  let replyHtml = '';
  if (data.replyTo) {
    const rName = escapeHtml(data.replyTo.senderName || 'مستخدم');
    const rText = data.replyTo.text
      ? escapeHtml(data.replyTo.text.length > 60 ? data.replyTo.text.slice(0, 60) + '...' : data.replyTo.text)
      : '🖼️ صورة';
    replyHtml = `
      <div class="reply-quote">
        <div class="reply-quote-bar"></div>
        <div class="reply-quote-content">
          <span class="reply-quote-name">${rName}</span>
          <span class="reply-quote-text">${rText}</span>
        </div>
      </div>`;
  }
  ...
  if (data.type === 'voice') {
    const dur = data.duration ? formatVoiceDuration(data.duration) : '';
    const bars = Array.from({length: 22}, () =>
      `<div class="voice-bar" style="height:${4 + Math.floor(Math.random() * 16)}px"></div>`
    ).join('');
    contentHtml = `
      <div class="voice-message" data-audio="${escapeAttr(data.audioUrl || '')}">
        <button class="voice-play-btn" onclick="toggleAudioPlay(this)">
          <svg class="play-icon" width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3"/></svg>
          <svg class="pause-icon" width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style="display:none"><rect x="6" y="4" width="4" height="16" rx="1"/><rect x="14" y="4" width="4" height="16" rx="1"/></svg>
        </button>
  ...
  if (isMe && readReceiptsEnabled) {
    ...
    if (status === 'sent') {
      readReceiptHtml = `<span class="read-receipt sent" title="مرسلة">
        <svg viewBox="0 0 16 12" fill="none" style="width:16px;height:12px"><path d="M2 6L6 10L14 2" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
      </span>`;
    } else if (status === 'delivered') {
      readReceiptHtml = `<span class="read-receipt delivered" title="مستلمة">
        <svg viewBox="0 0 22 12" fill="none" style="width:22px;height:12px"><path d="M2 6L6 10L14 2" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M8 6L12 10L20 2" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
      </span>`;
    } else {
      readReceiptHtml = `<span class="read-receipt read" title="مقروءة">
        <svg viewBox="0 0 22 12" fill="none" style="width:22px;height:12px"><path d="M2 6L6 10L14 2" stroke="#53BDEB" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M8 6L12 10L20 2" stroke="#53BDEB" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
      </span>`;
    }
  }
  ...
  wrap.addEventListener('contextmenu', (e) => {
    e.preventDefault();
    showContextMenu(e.clientX, e.clientY, id, isMe, data);
  });
  wrap.addEventListener('touchstart', (e) => {
    longPressTimer = setTimeout(() => {
      const touch = e.touches[0];
      showContextMenu(touch.clientX, touch.clientY, id, isMe, data);
    }, 500);
  }, { passive: true });
...
async function sendMessage() {
  ...
  // حفظ بيانات الرد ثم إلغاؤه من الـ UI
  const replyData = currentReply ? { ...currentReply } : null;
  cancelReply();

  // رسالة مؤقتة فورية
  const area   = document.getElementById('messages-area');
  const tempEl = buildTempMessage(text, replyData);
  ...
  try {
    ...
    await FB.addDoc(msgsRef, msgPayload);
...
function buildTempMessage(text, replyData) {
  const wrap = document.createElement('div');
  ...
  const replyHtml = replyData ? `
    <div class="reply-quote">
      <div class="reply-quote-bar"></div>
      <div class="reply-quote-content">
        <span class="reply-quote-name">${escapeHtml(replyData.senderName || '')}</span>
        <span class="reply-quote-text">${escapeHtml((replyData.text || '').slice(0, 60))}</span>
      </div>
    </div>` : '';
  ...
  wrap.innerHTML = `
    <div class="message-bubble sending">
      ...
      <div class="message-meta">
        ...
        <span class="read-receipt sent"><svg viewBox="0 0 16 12" fill="none" style="width:16px;height:12px"><path d="M2 6L6 10L14 2" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></span>
      </div>
    </div>`;
  return wrap;
}
...
function clearChatHistory() {
  ...
  // في المجموعات: حذف كامل | في المحادثات الفردية: مسح لي فقط
...
function _clearChatForMe() {
  showConfirmModal('سيتم إخفاء رسائل هذه المحادثة لك فقط. متأكد؟', async () => {
    showTopLoader(true);
    try {
      await FB.updateDoc(FB.doc(FB.db, 'chats', currentChatId), {
        [`clearedAt.${currentUser.uid}`]: FB.serverTimestamp()
      });
      // إخفاء الرسائل من الـ DOM فوراً
      const area = document.getElementById('messages-area');
      if (area) area.innerHTML = '';
      showToast('تم مسح المحادثة لك ✅', 'success');
    } catch (err) {
...
function _clearChatForGroup() {
  showConfirmModal('سيتم مسح جميع رسائل المجموعة. هل أنت متأكد؟', async () => {
    showTopLoader(true);
    try {
      const msgsRef = FB.collection(FB.db, 'chats', currentChatId, 'messages');
      ...
      showToast('تم مسح المجموعة ✅', 'success');
    } catch (err) {
      console.error('Clear group error:', err);
...
function showContextMenu(x, y, msgId, isMe, msgData) {
  contextMenuMsgId  = msgId;
  contextMenuIsMe   = isMe;
  ctxFullMsgData    = msgData;

  const menu = document.getElementById('msg-context-menu');
  if (!menu) return;

  // إظهار/إخفاء الأزرار بناءً على نوع الرسالة وصلاحية المستخدم
  const btnDeleteAll  = document.getElementById('ctx-delete-everyone-btn');
  const btnDeleteMe   = document.getElementById('ctx-delete-btn');
  const btnCopy       = document.getElementById('ctx-copy-btn');
  const btnReply      = document.getElementById('ctx-reply-btn');
  const btnForward    = document.getElementById('ctx-forward-btn');

  if (btnDeleteAll) btnDeleteAll.style.display  = isMe ? 'flex' : 'none';
  if (btnDeleteMe)  btnDeleteMe.style.display   = 'flex';
  if (btnCopy)      btnCopy.style.display       = (msgData.type === 'text' && msgData.text) ? 'flex' : 'none';
  if (btnReply)     btnReply.style.display      = 'flex';
  if (btnForward)   btnForward.style.display    = 'flex';
  ...
  menu.dataset.text = msgData.text || '';
...
// ── ربط أزرار قائمة السياق ──
document.addEventListener('DOMContentLoaded', () => {

  // ↩️ رد
  document.getElementById('ctx-reply-btn')?.addEventListener('click', () => {
    document.getElementById('msg-context-menu').style.display = 'none';
    if (!ctxFullMsgData || !contextMenuMsgId) return;
    const senderName = ctxFullMsgData.senderName
      || (contextMenuIsMe ? (currentUser?.name || 'أنت') : (currentPartner?.name || 'مستخدم'));
    const previewText = ctxFullMsgData.type === 'voice' ? '🎤 رسالة صوتية'
                      : ctxFullMsgData.type === 'image' ? '🖼️ صورة'
                      : (ctxFullMsgData.text || '');
    setReply(contextMenuMsgId, previewText, senderName);
  });

  // ↪️ إعادة توجيه
  document.getElementById('ctx-forward-btn')?.addEventListener('click', () => {
    document.getElementById('msg-context-menu').style.display = 'none';
    if (ctxFullMsgData) openForwardModal(ctxFullMsgData);
  });

  // 🗑️ حذف للجميع
  document.getElementById('ctx-delete-everyone-btn')?.addEventListener('click', () => {
    document.getElementById('msg-context-menu').style.display = 'none';
    if (contextMenuMsgId && contextMenuIsMe) deleteForEveryone(contextMenuMsgId);
  });

  // 🙈 مسح لي فقط (إخفاء من DOM)
  document.getElementById('ctx-delete-btn')?.addEventListener('click', () => {
    ...
    showToast('تم إخفاء الرسالة لك ✅', 'success');
  });
  ...
  document.getElementById('ctx-copy-btn')?.addEventListener('click', () => {
    ...
    if (text) {
      navigator.clipboard.writeText(text)
        .then(() => showToast('تم نسخ الرسالة 📋', 'success'))
...
/* ── حذف للجميع (isDeleted:true) ── */
async function deleteForEveryone(msgId) {
  if (!currentChatId) return;
  showConfirmModal('سيتم حذف هذه الرسالة للجميع. هل أنت متأكد؟', async () => {
    try {
      await FB.updateDoc(FB.doc(FB.db, 'chats', currentChatId, 'messages', msgId), {
        isDeleted: true,
...
function setReply(msgId, text, senderName) {
  currentReply = { id: msgId, text, senderName };

  const wrap   = document.getElementById('reply-preview-wrap');
  const nameEl = document.getElementById('reply-preview-name');
  const textEl = document.getElementById('reply-preview-text');

  if (nameEl) nameEl.textContent = senderName || 'مستخدم';
  if (textEl) {
...
async function loadForwardList() {
  ...
  container.innerHTML = '<div class="members-loading">⏳ تحميل المستخدمين...</div>';
  ...
  try {
    ...
    snap.forEach(d => {
      ...
      item.innerHTML = `
        <img src="${escapeAttr(u.avatar || generateAvatar(u.name || 'U'))}"
             alt="${escapeAttr(u.name || '')}"
             onerror="this.src='${generateAvatar(u.name || 'U')}'"/>
        <div class="forward-item-info">
          <div class="forward-item-name">${escapeHtml(u.name || 'مستخدم')}</div>
          <div class="forward-item-bio">${escapeHtml(u.bio || 'مستخدم ChatWave')}</div>
        </div>
        <span class="forward-status-badge${u.online ? ' online' : ''}">${u.online ? '🟢' : '⚫'}</span>`;
      item.addEventListener('click', () => forwardTo(u));
      container.appendChild(item);
    });

    if (!container.children.length) {
      container.innerHTML = '<div class="members-loading">لا يوجد مستخدمون آخرون</div>';
    }
  } catch (err) {
    container.innerHTML = '<div class="members-loading">خطأ في التحميل</div>';
  }
}

async function forwardTo(partner) {
  if (!forwardMsgData || !currentUser) return;
  closeForwardModal();
  showTopLoader(true);

  try {
    const chatId = buildChatId(currentUser.uid, partner.uid);
    await ensureChatExists(chatId, partner);
    ...
    let lastMsgPreview = '';
    if (forwardMsgData.type === 'image') {
      Object.assign(msgPayload, { type: 'image', imageUrl: forwardMsgData.imageUrl, caption: forwardMsgData.caption || '', text: '' });
      lastMsgPreview = '📷 صورة محوّلة';
    } else if (forwardMsgData.type === 'voice') {
      Object.assign(msgPayload, { type: 'voice', audioUrl: forwardMsgData.audioUrl, duration: forwardMsgData.duration || 0, text: '🎤 رسالة صوتية' });
      lastMsgPreview = '🎤 رسالة صوتية محوّلة';
    } else {
      Object.assign(msgPayload, { type: 'text', text: forwardMsgData.text || '' });
      lastMsgPreview = (forwardMsgData.text || '').slice(0, 50);
    }
    ...
    await FB.updateDoc(FB.doc(FB.db, 'chats', chatId), {
      ...
      updatedAt:   FB.serverTimestamp(),
      [`unreadCount.${partner.uid}`]: FB.increment(1),
    });

    showToast(`تم توجيه الرسالة إلى ${partner.name} ✅`, 'success');
  } catch (err) {
    console.error('Forward error:', err);
    showToast('فشل توجيه الرسالة ❌', 'error');
  } finally {
    showTopLoader(false);
  }
}
...
async function startVoiceRecord() {
  if (!currentChatId) { showToast('اختر محادثة أولاً ⚠️', 'error'); return; }
  if (!navigator.mediaDevices?.getUserMedia) { showToast('المتصفح لا يدعم التسجيل ❌', 'error'); return; }
  ...
  try {
    ...
    mediaRecorder.onstop = () => {
      ...
      if (!voiceCancelled && audioChunks.length) uploadAndSendVoice();
      else audioChunks = [];
    };
    ...
    // إخفاء input وإظهار شريط التسجيل
    document.getElementById('message-input-area').style.display = 'none';
    document.getElementById('voice-recording-bar').style.display = 'flex';
    document.getElementById('mic-btn')?.classList.add('recording');

    updateVoiceTimer();
    voiceTimerInt = setInterval(updateVoiceTimer, 1000);

  } catch (err) {
    console.error('Mic error:', err);
    showToast('تعذّر الوصول إلى الميكروفون. تحقق من الإذن ❌', 'error');
  }
}
...
function updateVoiceTimer() {
  ...
  if (el) el.textContent = formatVoiceDuration(voiceSeconds);
  if (voiceSeconds >= 300) stopAndSendVoice(); // حد 5 دقائق
}
...
async function uploadAndSendVoice() {
  if (!currentChatId || !currentUser || !audioChunks.length) return;
  ...
  if (blob.size > 10 * 1024 * 1024) { showToast('الرسالة الصوتية أكبر من 10MB ⚠️', 'error'); return; }

  // مؤشر مؤقت
  const area   = document.getElementById('messages-area');
  const loadEl = document.createElement('div');
  loadEl.className = 'message-wrap outgoing';
  loadEl.innerHTML = `<div class="message-bubble sending">
    <div class="upload-progress"><div class="upload-spinner"></div><span style="font-size:.82rem;color:rgba(255,255,255,.7)">جارٍ رفع الرسالة الصوتية...</span></div>
  </div>`;
  area.appendChild(loadEl);
  scrollToBottom(area);
  ...
  try {
    ...
    const storRef   = FB.ref(FB.storage, `chatwave/${currentChatId}/voice/${fileName}`);
    await FB.uploadBytesResumable(storRef, blob);
    const audioUrl  = await FB.getDownloadURL(storRef);
    ...
    await FB.updateDoc(FB.doc(FB.db, 'chats', currentChatId), {
      lastMessage: `🎤 رسالة صوتية (${formatVoiceDuration(duration)})`,
      updatedAt:   FB.serverTimestamp(),
      [`unreadCount.${currentPartner?.uid}`]: FB.increment(1),
    });
    ...
    const bubble = loadEl.querySelector('.message-bubble');
    if (bubble) bubble.classList.add('failed');
    showToast('فشل إرسال الرسالة الصوتية ❌', 'error');
  } finally {
    loadEl.remove();
  }
}
...
/* ══════════════════════════════════════════════════════
   🎵 تشغيل / إيقاف رسالة صوتية
══════════════════════════════════════════════════════ */
function toggleAudioPlay(btn) {
  const voiceEl = btn.closest('.voice-message');
  const url     = voiceEl?.dataset.audio;
  if (!url) { showToast('رابط الصوت غير متوفر', 'error'); return; }

  // إيقاف أي تشغيل آخر نشط
  if (activeAudioEl && activePlayBtn && activePlayBtn !== btn) {
    activeAudioEl.pause();
    activeAudioEl.currentTime = 0;
    _resetVoiceBtn(activePlayBtn);
  }

  // إنشاء عنصر الصوت مرة واحدة لكل زر
  if (!btn._audio) {
    btn._audio = new Audio(url);
    btn._audio.onended = () => _resetVoiceBtn(btn);
    btn._audio.onerror = () => { showToast('فشل تشغيل الصوت ❌', 'error'); _resetVoiceBtn(btn); };
  }
  ...
  if (btn._audio.paused) {
    btn._audio.play()
      .then(() => {
        ...
        voiceEl.querySelectorAll('.voice-bar').forEach(b => b.classList.add('playing'));
        activeAudioEl = btn._audio;
        activePlayBtn = btn;
      })
      .catch(() => showToast('فشل تشغيل الصوت ❌', 'error'));
  } else {
    btn._audio.pause();
    _resetVoiceBtn(btn);
  }
...
function _resetVoiceBtn(btn) {
  ...
  btn.closest?.('.voice-message')?.querySelectorAll('.voice-bar').forEach(b => b.classList.remove('playing'));
  if (activePlayBtn === btn) { activeAudioEl = null; activePlayBtn = null; }
}
...
async function openCreateGroupModal() {
  ...
  document.getElementById('selected-members-preview').style.display = 'none';
    ...
    snap.forEach(d => {
      ...
      item.innerHTML = `
        <div class="group-member-check">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none"><path d="M20 6L9 17 4 12" stroke="white" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/></svg>
        </div>
        <img class="group-member-avatar" src="${escapeAttr(u.avatar || generateAvatar(u.name || 'U'))}" alt="${escapeAttr(u.name || 'U')}" onerror="this.src='${generateAvatar(u.name || 'U')}'"/>
        <div class="group-member-info">
          <div class="group-member-name">${escapeHtml(u.name || 'مستخدم')}</div>
          <div class="group-member-bio">${escapeHtml(u.bio || 'مستخدم ChatWave')}</div>
        </div>
      `;
      item.addEventListener('click', () => toggleGroupMember(u, item));
      container.appendChild(item);
    });
...
function toggleGroupMember(user, item) {
  if (selectedGroupMembers.has(user.uid)) {
    selectedGroupMembers.delete(user.uid);
...
function updateSelectedMembersPreview() {
  ...
  const avatarEl = document.getElementById('selected-avatars');
  if (!preview) return;
  ...
  if (avatarEl) {
    avatarEl.innerHTML = '';
    const items = document.querySelectorAll('.group-member-item.selected');
    items.forEach(item => {
      const img = item.querySelector('.group-member-avatar');
      const name = item.querySelector('.group-member-name')?.textContent || '';
      const chip = document.createElement('div');
      chip.className = 'selected-avatar-chip';
      chip.innerHTML = `<img src="${img.src}" alt="${escapeAttr(name)}"/>${escapeHtml(name)}`;
      avatarEl.appendChild(chip);
    });
  }
}
...
async function createGroup() {
  ...
  if (!name) { showToast('أدخل اسم المجموعة ⚠️', 'error'); return; }
  if (selectedGroupMembers.size < 1) { showToast('اختر عضواً واحداً على الأقل ⚠️', 'error'); return; }

  showTopLoader(true);
  try {
    const members = [currentUser.uid, ...selectedGroupMembers];
    const memberNames   = { [currentUser.uid]: currentUser.name };
    const memberAvatars = { [currentUser.uid]: currentUser.avatar };
    ...
    for (const uid of selectedGroupMembers) {
      ...
      if (snap.exists()) {
        const d = snap.data();
        memberNames[uid]   = d.name || 'مستخدم';
        memberAvatars[uid] = d.avatar || generateAvatar(d.name || 'U');
      }
    }
    ...
    const groupRef = await FB.addDoc(FB.collection(FB.db, 'chats'), {
      isGroup:       true,
      groupName:     name,
      groupDesc:     desc,
      groupAvatar:   `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=25D366&color=fff&size=200&bold=true`,
      members,
      memberNames,
      memberAvatars,
      adminUid:      currentUser.uid,
      lastMessage:   `📢 ${currentUser.name} أنشأ المجموعة`,
      updatedAt:     FB.serverTimestamp(),
      createdAt:     FB.serverTimestamp(),
      unreadCount,
    });
    ...
    showToast(`تم إنشاء مجموعة "${name}" ✅`, 'success');
    switchTab('chats');
    ...
    openChat(groupRef.id, {
      uid: groupRef.id,
      name,
      avatar: memberAvatars[currentUser.uid],
      isGroup: true,
    });

  } catch (err) {
    console.error('Create group error:', err);
    showToast('فشل إنشاء المجموعة ❌', 'error');
  } finally {
    showTopLoader(false);
  }
}
...
/* ══════════════════════════════════════════════════════
   📖 تمييز الرسائل كمقروءة (قراءة حقيقية)
══════════════════════════════════════════════════════ */
async function markMessagesAsRead(chatId) {
  if (!chatId || !currentUser || !currentPartner) return;
  try {
    const msgsRef = FB.collection(FB.db, 'chats', chatId, 'messages');
    const q = FB.query(msgsRef, FB.orderBy('createdAt', 'desc'), FB.limit(30));
    const snap = await FB.getDocs(q);
    const batch = FB.writeBatch(FB.db);
    let updated = false;
    snap.docs.forEach(d => {
      const data = d.data();
      // رسائل الآخرين التي لم يُقرأها currentUser
      if (data.senderId !== currentUser.uid) {
        const readBy = data.readBy || [];
        if (!readBy.includes(currentUser.uid)) {
          batch.update(d.ref, { readBy: FB.arrayUnion(currentUser.uid), delivered: true });
          updated = true;
        }
      }
    });
