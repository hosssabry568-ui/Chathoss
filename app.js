...
function buildMessageElement(id, data, isMe, date) {
  ...
  // علامات القراءة بأسلوب واتساب (SVG مزدوجة)
  let readReceiptHtml = '';
  if (isMe && readReceiptsEnabled) {
    ...
    if (status === 'sent') {
      ...
      readReceiptHtml = `<span class="read-receipt sent" title="مرسلة">
        <svg viewBox="0 0 16 12" fill="none" xmlns="http://www.w3.org/2000/svg" style="width:16px;height:12px">
          <path d="M2 6L6 10L14 2" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
      </span>`;
    } else if (status === 'delivered') {
      // علامتان ✓✓ رماديتان
      readReceiptHtml = `<span class="read-receipt delivered" title="مستلمة">
        <svg viewBox="0 0 22 12" fill="none" xmlns="http://www.w3.org/2000/svg" style="width:22px;height:12px">
          <path d="M2 6L6 10L14 2" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
          <path d="M8 6L12 10L20 2" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
      </span>`;
    } else {
      // علامتان ✓✓ زرقاوان
      readReceiptHtml = `<span class="read-receipt read" title="مقروءة">
        <svg viewBox="0 0 22 12" fill="none" xmlns="http://www.w3.org/2000/svg" style="width:22px;height:12px">
          <path d="M2 6L6 10L14 2" stroke="#53BDEB" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
          <path d="M8 6L12 10L20 2" stroke="#53BDEB" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
      </span>`;
    }
  }
  ...
  // قائمة السياق بالضغط المطول أو كليك يمين
  wrap.addEventListener('contextmenu', (e) => {
    e.preventDefault();
    showContextMenu(e.clientX, e.clientY, id, isMe, data.text);
  });
...
function handleTouchStart(e) {
  ...
  const isMe   = wrap.classList.contains('outgoing');
  const textEl = wrap.querySelector('.message-text');
  const text   = textEl ? textEl.textContent : '';

  longPressTimer = setTimeout(() => {
    const touch = e.touches[0];
    showContextMenu(touch.clientX, touch.clientY, msgId, isMe, text);
  }, 500);
}
...
function showContextMenu(x, y, msgId, isMe, text) {
  ...
  // إظهار/إخفاء زر المسح (فقط للمرسل)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (deleteBtn) {
    deleteBtn.addEventListener('click', () => {
      ...
      if (contextMenuMsgId && contextMenuIsMe) deleteSingleMessage(contextMenuMsgId);
    });
  }
  if (copyBtn) {
...
async function deleteSingleMessage(msgId) {
  if (!currentChatId) return;
  showConfirmModal('هل تريد حذف هذه الرسالة؟', async () => {
    try {
      const msgRef = FB.doc(FB.db, 'chats', currentChatId, 'messages', msgId);
      await FB.deleteDoc(msgRef);
